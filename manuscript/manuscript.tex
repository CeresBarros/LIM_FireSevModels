...
\begin{Shaded}
\begin{Highlighting}[]
\AlertTok{![A meaningless scatterplot](path/to/figure.png)}\NormalTok{\{\#fig{-}meaningless\}}

\NormalTok{See @fig{-}meaningless.}
\end{Highlighting}
\end{Shaded}

Or as a plain markdown table with \texttt{\#\textbar{}\ label} / caption
syntax (see Table~\ref{tbl-simple}).

\begin{longtable}[]{@{}llll@{}}
\caption{Caption centered above table.}\label{tbl-simple}\tabularnewline
\toprule\noalign{}
mpg & cyl & disp & hp \\
\midrule\noalign{}
\endfirsthead
...
executable R chunks (\texttt{knitr::kable()}, \texttt{plot()}); this
repo's R installation does not currently have
\texttt{knitr}/\texttt{rmarkdown} on its library path, so the chunks
were replaced with static equivalents. Install those packages if
executable R chunks are needed later.
